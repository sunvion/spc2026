...
- 검색 결과가 한 번에 안나올 경우, 유사어(유의어) 등으로 변경해서 재시도 할 수 있음

영어 검색한 결과인 경우, 한국어로 번역해서 답변하시오.
"""

agent = create_agent(llm, [wiki_en, wiki_ko], system_prompt=system_prompt)

questions = ["파이썬은 누가 만든 언어야?", "What is artificial intelligence?", "세종대왕은 누구인가요?"]
# result = agent.invoke({"messages": [("user", questions[0])]})
# # print(result)
