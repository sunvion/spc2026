...
first_input = "저의 직업의 프로그래머입니다."
result5 = app.invoke({"messages": [HumanMessage(content=first_input)]}, config=config1)

print(f"AI 응답: {result5['messages'][-1].content}")

second_input = "제 이름은 무엇이고, 저는 무슨 일을 하나요?"
result6 = app.invoke({"messages": [HumanMessage(content=second_input)]}, config=config1)

print(f"AI 응답: {result6['messages'][-1].content}")

second_input = "제 이름은 무엇이고, 저는 무슨 일을 하나요?"
result7 = app.invoke({"messages": [HumanMessage(content=second_input)]}, config=config2)
