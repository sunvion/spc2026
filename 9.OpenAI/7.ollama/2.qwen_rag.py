...
def rag_query(user_query):
    ...
    distance, indices = index.search(np.array([query_embedding]), k=1) # 백터DB에서 (user_query)의 숫자값과 가장 가까운 k=1 개를 반환하시오.
...
query = '파이썬은 어떤 언어인가요?'
# query = '오늘 저녁은 뭐 먹을까?'
# query = 'JavaScript은 개발 언어 중에 가장 쉽다고 하는데, 그렇게 쉬운 언어는 아닙니다.?'
